...
// Animation duration in ms — must match CSS --pin-duration
...
export function LocationPinOverlay() {
    ...
    useEffect(() => {
        const unsub = dataBus.on("showLocationPin", () => {
            setVisible(true);
            const t = setTimeout(() => setVisible(false), ANIM_DURATION_MS);
    ...
    return (
        ...
        >
            ...
            <style>{`
                ...
                .location-pin-icon {
                    position: absolute;
                    /* Shift up so the pin tip lands at screen center */
